...
if __name__ == "__main__":
    with open("./input.txt", "r") as f:
        ...
        last_number = 0
        winner_board = Board.empty_board()
        for number in numbers:
            for board in boards.values():
                ...
                if board.is_winner_board():
                    winner_board = board
                    last_number = number
                    logging.info(f"The winner board is board number {winner_board.number}")
                    logging.info(f"The board is {winner_board.rows}")
        ...
        logging.info(f"The winner board is board number {winner_board.number}")
        logging.info(f"The board is {winner_board.rows}")
        logging.debug(f"The flatten board is {winner_board.flatten}")
        not_matched = winner_board.total_not_matched()
